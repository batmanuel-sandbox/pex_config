...
class ConfigDict(Dict):
    ...
    Much like `Dict`, `ConfigDict` is a custom `MutableMapper` which tracks the history of changes to any of
    its items.
    """
...
class ConfigDictField(DictField):
    """Configuration field that is a mapping between keys and `lsst.pex.Config`-type classes.
    ...
    ValueError
        ...
        - ``keytype`` or ``itemtype`` arguments are not supported types (members of
          `ConfigDictField.supportedTypes`.
        - ``dictCheck`` or ``itemCheck`` is not a callable function.

    Notes
    -----
    ``ConfigDictField`` behaves like `DictField` except that the ``itemtype`` must be an subclass of
    `lsst.pex.Config`.

    You can use ``ConfigDictField`` to create name-to-config mappings. One use case is for configuring
    mappings for dataset types in a butler. In this case, the dataset type names are arbitrary and
    user-selected while the mapping configurations are known and fixed.
    """
